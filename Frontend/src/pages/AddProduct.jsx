...
const EMPTY = {
  ...
  supplierName: '', supplierContact: '', supplierEmail: '', reorderThreshold: '5',
};
...
export default function AddProduct() {
  ...
  const validate = () => {
    ...
    if (!form.costPrice || isNaN(form.costPrice) || +form.costPrice < 0) e.costPrice = 'Enter a valid cost price.';
    ...
    else if (!/^\d{10}$/.test(form.supplierContact.trim()))
      e.supplierContact = '10-digit number required.';
    if (!form.supplierEmail.trim())   e.supplierEmail = 'Required.';
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.supplierEmail.trim()))
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      // barcode is returned from the backend — no client-side generation
      const p = await addProduct(form);
      setLastAdded(p);
      setForm(EMPTY);
      setErrors({});
      showToast(`"${p.name}" added — barcode: ${p.barcode}`);
    } catch (err) {
    ...
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Add a new product to inventory — a barcode is auto-generated by the server.
        </p>
      </div>
      ...
      <div className="add-product-layout">
            ...
            <div className="form-grid">
              ...
              <Field label="Selling Price (₹) *" error={errors.price}>
                <input className={`form-input ${errors.price ? 'input-error' : ''}`}
                  ...
                  value={form.price} onChange={handleChange} placeholder="Price charged to customer" />
              </Field>

              {/* Cost price */}
              <Field label="Cost Price (₹) *" error={errors.costPrice}>
                <input className={`form-input ${errors.costPrice ? 'input-error' : ''}`}
                  name="costPrice" type="number" min="0" step="0.01"
                  value={form.costPrice} onChange={handleChange} placeholder="Price store pays to acquire" />
              </Field>
              ...
              {/* Supplier email — required for auto low-stock + reorder emails */}
              <Field label="Supplier Email *" error={errors.supplierEmail} full>
                <input className={`form-input ${errors.supplierEmail ? 'input-error' : ''}`}
                  name="supplierEmail" type="email" value={form.supplierEmail}
                  onChange={handleChange}
                  placeholder="supplier@example.com — used for automated alerts & reorder emails" />
              </Field>
        ...
        {/* ── Barcode result (returned by server) ─────────────────────── */}
        {lastAdded && (
          <div className="card barcode-card" ref={barcodeRef}>
            ...
            <div className="barcode-meta">
              ...
              <span>Sell ₹{lastAdded.price}</span>
              <span>Cost ₹{lastAdded.costPrice}</span>
            ...
            <tbody>
              {products.map((p) => (
                <tr key={p.id} className={p.stock < p.reorderThreshold ? 'row-low-stock' : ''}>
                  ...
                  <td className="text-right text-muted">₹{p.costPrice?.toLocaleString() ?? '—'}</td>
                  ...
                  <td className="text-muted text-sm">{p.supplierEmail || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
